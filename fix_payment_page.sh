...
import { CheckCircle2, XCircle, RefreshCcw, Loader2, BookOpen, GraduationCap } from "lucide-react"
...
function PaymentResultContent() {
    ...
    useEffect(() => {
        if (success && !pending) {
            ...
            const pendingCheckout = consumePendingCheckout() as any
            if (pendingCheckout) {
                trackEvent('purchase', {
...
echo "✅ Payment result page fixed!"
