...
export const runRbacSeeder = async (dataSource: DataSource) => {
    ...
    const defaultPermissions = [
        ...
        { name: 'Manage Admins', action: 'manage', resource: 'admins', description: 'إضافة/تعديل/إيقاف المشرفين' },
        { name: 'View Roles', action: 'view', resource: 'roles', description: 'عرض الأدوار والصلاحيات' },
        { name: 'Manage Roles', action: 'manage', resource: 'roles', description: 'إدارة الأدوار والصلاحيات' },
        { name: 'View Audit', action: 'view', resource: 'audit', description: 'عرض سجل تدقيق عمليات الإدارة' },
        ...
        { name: 'View Courses', action: 'view', resource: 'courses', description: 'عرض المحتوى التعليمي' },
        { name: 'Manage Courses', action: 'manage', resource: 'courses', description: 'إدارة المحتوى التعليمي' },
        { name: 'View Payments', action: 'view', resource: 'payments', description: 'عرض المدفوعات' },
        { name: 'Manage Payments', action: 'manage', resource: 'payments', description: 'إدارة المدفوعات والاشتراكات' },
        { name: 'Manage Settings', action: 'manage', resource: 'settings', description: 'إدارة إعدادات المنصة' },
    ];
    ...
    for (const p of defaultPermissions) {
        let exists = await permissionRepo.findOne({ where: { action: p.action, resource: p.resource } });
        if (!exists) {
    ...
    let superAdminRole = await roleRepo.findOne({ where: { name: 'Super Admin' }, relations: ['permissions'] });
    if (!superAdminRole) {
    ...
    if (!supportRole) {
        const supportPermissions = savedPermissions.filter(p =>
