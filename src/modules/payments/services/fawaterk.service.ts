...
export class FawaterkService {
  ...
  async initiatePayment(request: FawaterkPaymentRequest): Promise<any> {
    try {
      ...
      const response = await axios.post(`${this.config.baseUrl}/createInvoiceLink`, payload, {
        headers: {
  ...
  async getInvoiceStatus(invoiceId: string): Promise<any> {
    try {
      const response = await axios.get(`${this.config.baseUrl}/getInvoiceStatus/${invoiceId}`, {
        headers: {
          Authorization: `Bearer ${this.config.apiKey}`,
        },
      });
