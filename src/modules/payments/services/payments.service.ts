...
export class PaymentsService {
    ...
    async createSessionCheckout(sessionId: string, userId: string): Promise<any> {
        ...
        return this.processFawaterkPayment(transaction, user,
    ...
    private async processFawaterkPayment(transaction: Transaction, user: any, itemName: string, payload: any) {
        ...
        if (status === 'paid' && transaction.status !== TransactionStatus.SUCCESS) {
            ...
            try {
                ...
                if (transaction.type === PaymentType.SESSION_BOOKING) {
                    ...
                    await this.subscriptionsService.createSubscriptionFromPayment(transaction.userId, transaction.referenceId, transaction.id);
                } else if (transaction.type === PaymentType.COURSE_ENROLLMENT) {
                    await this.contentService.enrollUserInCourse(transaction.referenceId, transaction.userId, EnrollmentSource.PAYMENT, transaction.id);
                    this.logger.log(`Course enrollment fulfilled for user ${transaction.userId}, course ${transaction.referenceId}`);
                }

                // Mark as fulfilled to prevent double-fulfillment on webhook replay
                await this.transactionRepository.update(transaction.id, {
                    metadata: { ...((transaction.metadata as any) || {}), fulfilled: true, fulfilledAt: new Date().toISOString() },
                });
            } catch (error) {
                this.logger.error(`Failed to fulfill order for transaction ${transaction.id}`, error);
                await this.transactionRepository.update(transaction.id, {
                    metadata: { ...((transaction.metadata as any) || {}), fulfillmentError: (error as Error).message, fulfillmentFailedAt: new Date().toISOString() },
                });
            }
