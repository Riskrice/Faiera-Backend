...
export class ProgressService {
    ...
    async getProgress(userId: string, contentType: ContentType, contentId: string): Promise<UserProgress | null> {
    ...
    async getCourseProgress(userId: string, courseId: string): Promise<{
        ...
        const lessonsProgress = await this.progressRepository.find({
            where: {
                userId,
        ...
        // Filter by courseId stored in metadata
        const courseLessons = lessonsProgress.filter(
            p => p.metadata && (p.metadata as any).courseId === courseId
        );
