...
export class ProgressController {
  ...
  async getMyCourseProgress(
    ...
    ApiResponse<{ totalLessons: number; completedLessons: number; overallProgress: number }>
  > {
   ...
   */
  @Get('my/stats')
  async getMyStats(@CurrentUser() user: JwtPayload): Promise<
    ApiResponse<{
  ...
  > {
    ...
    const completedCourses = [...courseMap.values()].filter(
    ...
    const progressDates = new Set(
      allProgress
        .map(p => (p.updatedAt ? new Date(p.updatedAt).toDateString() : null))
        .filter(Boolean),
    );
    let streakDays = 0;
    const d = new Date();
