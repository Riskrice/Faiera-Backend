...
export class NotificationsController {
    ...
    @Post('send/template')
    @Roles(Role.ADMIN, Role.SUPER_ADMIN)
    async sendFromTemplate(@Body() dto: SendTemplateNotificationDto): Promise<ApiResponse<Notification>> {
    ...
    async getMyNotifications(
        ...
        const { notifications, total, unreadCount } = await this.notificationsService.getUserNotifications(
            user.sub,
            query,
    ...
    @Get('preferences')
    async getPreferences(@CurrentUser() user: JwtPayload): Promise<ApiResponse<NotificationPreference>> {
