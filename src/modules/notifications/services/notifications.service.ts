...
export class NotificationsService {
  ...
  private async sendPush(notification: Notification): Promise<void> {
    ...
    this.logger.warn(
      `Push notification ${notification.id} queued but FCM not configured — skipping delivery`,
    );
  ...
  private async sendSms(notification: Notification): Promise<void> {
    ...
    // Until configured, mark as pending instead of falsely marking delivered
    this.logger.warn(
      `SMS notification ${notification.id} queued but Twilio not configured — skipping delivery`,
    );
