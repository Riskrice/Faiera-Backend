...
export class ContentService {
          ...
          if (sectionLessons && sectionLessons.length > 0) {
            for (let j = 0; j < sectionLessons.length; j++) {
              ...
              if (lessonDto.videoId) {
                ...
                videoResource = await this.videoRepository.findOne({
                  where: { bunnyVideoId: bunnyId },
                });
                if (!videoResource) {
  ...
  async findAllCourses(
    ...
    queryBuilder.skip(pagination.skip).take(pagination.take).orderBy('course.createdAt', 'DESC');
      ...
      if (sections) {
        ...
        for (let i = 0; i < sections.length; i++) {
          ...
          if (sanitizedSectionId) {
            module =
              existingModules.find(m => m.id === sanitizedSectionId) ||
              moduleRepo.create({ id: sanitizedSectionId });
          ...
          if (sectionDto.lessons) {
            for (let j = 0; j < sectionDto.lessons.length; j++) {
              ...
              if (sanitizedLessonId) {
                lesson =
                  existingLessons.find(l => l.id === sanitizedLessonId) ||
                  lessonRepo.create({ id: sanitizedLessonId });
  ...
  private async updateCourseStats(moduleId: string): Promise<void> {
    ...
    if (course) {
      course.lessonCount = lessons.length;
      course.totalDurationMinutes = lessons.reduce((sum, l) => sum + (l.durationMinutes || 0), 0);
      await this.courseRepository.save(course);
    }
  }
