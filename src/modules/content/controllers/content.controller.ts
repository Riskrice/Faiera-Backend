...
export class ContentController {
  ...
  async findAllPrograms(@Query() query: ProgramQueryDto): Promise<PaginatedResponse<Program>> {
  ...
  @Get('programs/all')
  @Public()
  async findAllProgramsAlias(@Query() query: ProgramQueryDto): Promise<PaginatedResponse<Program>> {
  ...
  > {
    ...
    if (!normalizedTitle && !normalizedVideoId) {
      throw new BadRequestException(
        'Either title or videoId is required for upload URL generation',
      );
    }
  ...
  async getStreamUrl(
    ...
    if (!lesson.isFree) {
      ...
      const isPrivileged = [Role.ADMIN, Role.SUPER_ADMIN, Role.TEACHER].includes(user.role);
      if (!isPrivileged) {
    ...
    if (lesson.video.status !== VideoStatus.READY) {
      ...
      this.logger.warn(
        `Video ${lesson.video.bunnyVideoId} status is ${lesson.video.status}, attempting playback anyway`,
      );
    }
  ...
  @Get('enrollments/my')
  async getMyEnrollments(@CurrentUser() user: JwtPayload): Promise<ApiResponse<any[]>> {
  ...
  async enrollFreeCourse(
    ...
    if (course.price && course.price > 0) {
      throw new BadRequestException(
        'This course is not free. Please complete the checkout process.',
      );
    }
