...
import {
  AssessmentAttempt,
  AttemptStatus,
} from '../../assessments/entities/assessment-attempt.entity';
import {
  OverviewStatsDto,
  UserStatsDto,
  LearningStatsDto,
...
export class AnalyticsService {
  ...
  async getOverview(): Promise<OverviewStatsDto> {
    ...
    // User counts
    const [totalUsers, totalStudents, totalTeachers, studentsWithAcademicProfile] =
      await Promise.all([
    ...
    const academicProfileCompletionRate =
      totalStudents > 0 ? Math.round((studentsWithAcademicProfile / totalStudents) * 1000) / 10 : 0;
