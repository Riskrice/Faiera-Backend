...
export class SubscriptionsService {
    ...
    async createSubscriptionFromPayment(userId: string, planId: string, transactionId: string): Promise<Subscription> {
    ...
    async createSubscription(dto: CreateSubscriptionDto): Promise<Subscription> {
        ...
        if (existingActive && !existingActive.isExpired()) {
            throw new BadRequestException(
                'User already has an active subscription for this grade',
            );
        }
