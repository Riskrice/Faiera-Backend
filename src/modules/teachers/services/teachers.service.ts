...
export class TeachersService {
  ...
  async createTeacherAccount(
    dto: CreateTeacherFullDto,
  ): Promise<{ user: any; profile: TeacherProfile }> {
  ...
  async getTeacherBookings(userId: string, status?: BookingStatus): Promise<TeacherBooking[]> {
    const profile = await this.findProfileByUserId(userId);
  ...
  async requestWithdrawal(
    ...
    const result = await this.profileRepository
      ...
      .where('"userId" = :userId AND "currentBalance" >= :amt', {
