...
export class UsersController {
  ...
  async findAll(@Query() query: UserQueryDto): Promise<PaginatedResponse<UserResponse>> {
    const { users, total } = await this.usersService.findAll(query, query);
    return createPaginatedResponse(users, query.page || 1, query.pageSize || 20, total);
  }
  ...
  async getMyStudents(
    @CurrentUser() currentUser: JwtPayload,
  ): Promise<ApiResponse<ParentWithStudents>> {
  ...
  async getMyParent(
    @CurrentUser() currentUser: JwtPayload,
  ): Promise<ApiResponse<StudentWithParent>> {
