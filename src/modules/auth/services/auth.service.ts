...
export class AuthService {
    ...
    if (existingUser) {
      if (existingUser.status === UserStatus.PENDING) {
        ...
        existingUser.preferredLanguage =
          dto.preferredLanguage || existingUser.preferredLanguage || 'ar';
        ...
        this.logger.log(
          `Updated pending account and activated registration: ${existingUser.email}`,
        );
  ...
  async verifyOtp(
    ...
    if (attempts !== null && attempts >= this.maxLoginAttempts) {
      throw new UnauthorizedException(
        'Account temporarily locked due to too many failed OTP attempts. Try again in 15 minutes.',
      );
    }
    ...
    if (user.status !== UserStatus.PENDING) {
      throw new UnauthorizedException(
        'This verification code is only valid for account activation',
      );
    }
  ...
  async changePassword(
    ...
    const hasPasswordSet =
      user.metadata?.hasPassword === true ||
      (user.metadata?.hasPassword === undefined && !user.googleId);
  ...
  private async issueOtpForUser(
    user: User,
    purpose: 'verification' | 'login' = 'login',
  ): Promise<void> {
