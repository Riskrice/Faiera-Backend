...
export class JwtStrategy extends PassportStrategy(Strategy, 'jwt') {
    ...
    private async validateLocalPayload(payload: JwtPayload): Promise<JwtPayload> {
        this.logger.debug(`Validating local payload for sub: ${payload.sub}, email: ${payload.email}, role: ${payload.role}`);
    ...
    private async validateSupabasePayload(payload: any): Promise<JwtPayload> {
        ...
        let user = await this.supabaseAuthService.getUserBySupabaseId(supabaseId);
