...
export class BunnyNetService {
  ...
  async createVideo(title: string, collectionId?: string): Promise<BunnyUploadCredentials> {
    ...
    const response = await axios.post(`${this.baseUrl}/library/${this.libraryId}/videos`, body, {
  ...
  async getVideoInfo(videoId: string): Promise<BunnyVideoInfo | null> {
    ...
    try {
      ...
      return {
        ...
        availableResolutions: ((data.availableResolutions as string) || '')
          .split(',')
          .filter(Boolean),
  ...
  async deleteVideo(videoId: string): Promise<void> {
    ...
    await axios.delete(`${this.baseUrl}/library/${this.libraryId}/videos/${videoId}`, {
      headers: { AccessKey: this.apiKey },
    });
