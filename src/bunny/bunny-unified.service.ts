...
export type BunnyWebhookVerificationStrategy =
    | 'security-token'
    | 'header-signature'
    | 'none';
...
export class BunnyUnifiedService {
    ...
    constructor(private readonly configService: ConfigService) {
        this.baseUrl = this.configService.get<string>('BUNNY_API_BASE_URL', 'https://video.bunnycdn.com');
    ...
    async getUploadCredentials(input: BunnyGetUploadCredentialsInput): Promise<BunnyUploadCredentials> {
        this.ensureConfigured();
        ...
        if (!title) {
            throw new BadRequestException(
                'title is required when no reusable Bunny video was found',
            );
        }
    ...
    verifyWebhook(input: BunnyWebhookVerificationInput): BunnyWebhookVerificationResult {
        ...
        const payloadSecurityToken = typeof payload.SecurityToken === 'string'
            ? payload.SecurityToken.trim()
            : '';
